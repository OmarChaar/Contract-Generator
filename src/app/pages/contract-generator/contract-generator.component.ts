...
export class ContractGeneratorComponent implements OnInit {
  ...
  show(id: any, apt?: number) {
    // console.log("this.parsedClients", this.parsedClients);
    // for(let i=0; i<this.parsedClients.length; i++) {
  ...
  setPISOS(personalization: any) {
    ...
    let pisosExtra = '';
    if(personalization['PISOS - EXTRA'] == 'NÃO (OP.1)') {
      pisosExtra = 'Op. 1 - sem piso a mais';
    }
    else if(personalization['PISOS - EXTRA'] == 'OP. 2') {
      pisosExtra = 'Op. 2 - 2m² a mais';
    }
    else if(personalization['PISOS - EXTRA'] == 'OP. 3') {
  ...
  setCubas(personalization: any) {
    ...
    tempObj['TANQUE ÁREA DE SERVIÇO'] = personalization['SIM KIT CHURRASQUEIRA - TANQUE ÁREA DE SERVIÇO - NOME DA OPÇÃO'] ? personalization['SIM KIT CHURRASQUEIRA - TANQUE ÁREA DE SERVIÇO - NOME DA OPÇÃO'] : 'Tanque 18L 50x38cm Branco Gelo - Deca';
